          ...
          <table>
            {credits.cast.map((data) => (
              <tbody key={uuidv4()}>
                <tr>
                  <td>
                    <img src={data.profile_path ? `${IMAGE_URL}${data.profile_path}` : 'http://via.placeholder.com/54x81'} alt="" />
                  </td>
                  <td>{data.name}</td>
        ...
        <div className="overview-detail">
          ...
          {details.production_companies.map((prod) => (
            <div className="product-company" key={uuidv4()}>
              <img src={prod.logo_path ? `${IMAGE_URL}${prod.logo_path}` : 'http:///via.placeholder.com/30x30'} alt="" />
              <span>{prod.name}</span>
            </div>
          ))}
        </div>
